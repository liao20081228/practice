...
protected:
    ...
    static const int padding_size = BOOST_LOCKFREE_CACHELINE_BYTES - sizeof(size_t); //缓存行为64
    atomic<size_t> write_index_;
    char padding1[padding_size]; /* force read_index and write_index to different cache lines */
    atomic<size_t> read_index_;

    BOOST_DELETED_FUNCTION(ringbuffer_base(ringbuffer_base const&)) // fun() = delete
...
protected:
    ...
    {
        size_t ret = arg + 1;
        while (BOOST_UNLIKELY(ret >= max_size)) // rear =  (rear+1)% MaxSize,因为取模运算花费的时间更高
            ret -= max_size;                    // 但是实际测试没有什么太大区别
        return ret;
    }
