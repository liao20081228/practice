...
def login(username,passwd):
    ...
    req=requests.post(API_HOST + '/api/v1/user/login',post_data)
    ret = json.loads(req.content.decode("utf-8"))
    if ret['status']: 
        return ret['key']
    else:
        print("[Error] failed to login:",ret['error'])
        sys.exit(1)

#judge_exist_station
def judge_station_exsisted(station_name,usr_key):
    get_data = { 'key':usr_key }
    req=requests.get(API_HOST + '/api/v1/site/get',get_data)
    ret = json.loads(req.content.decode("utf-8"))
    if ret[ 'status' ] :
        stations=ret['data']
        if not len(stations) :
        ...
        print("[Error] failed to get all station info:",ret['error'])
        sys.exit(1)

# Add Site
def add_site(site_name, administrator, school, address, chinese_name, longitude, latitude, collector_number,usr_key):
    post_data = {
        'name': site_name,
...
if __name__ == '__main__':
    usr_key=login(USER_NAME,USER_PASSWD)
    print(usr_key)
    if not judge_station_exsisted("huyanglou",usr_key):
        add_site("huyanglou","DSLab","LZU","test@126.com","兰州大学胡杨楼站","103.8599","36.0459","10",usr_key)
