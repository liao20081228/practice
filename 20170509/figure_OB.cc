...
{
	public:
		CFigure(std::function<void()>Display = nullptr)
			:__cm_pfnDisplay(Display)
		{
		}
		void OnDisplay()
		{
		...
		void SetDispaly(std::function<void()> Display)
		{
			__cm_pfnDisplay = Display;
		}
	private:
		std::function<void()> __cm_pfnDisplay;
};
class CCircle
{
	private:
...
{
	CCircle circle(10);
	CRectangle rectangle(10,20);
	CTriangle triangle(10,10,10);

	CFigure ob;
	ob.SetDispaly(std::bind(&CCircle::DoDisplay,&circle));
	ob.OnDisplay();
	ob.SetDispaly(std::bind(&CRectangle::DoDisplay,&rectangle));
	ob.OnDisplay();
	ob.SetDispaly(std::bind(&CTriangle::DoDisplay,&triangle));
	ob.OnDisplay();
